...
class BookScraper(CrawlSpider):
    ...
    rules = (
        Rule(LinkExtractor(restrict_css=".side_categories > ul > li > ul > li > a"), follow=True),
    ...
    def parse_book(self, response):
        ...
        book_item["img_url"] = response.urljoin(response.css(".item.active > img::attr(src)").get())
        ...
        self.Save_to_file(book_item["img_url"],book_item["title"], book_item["price"],book_item["upc"], book_item["url"]  )
        return book_item
